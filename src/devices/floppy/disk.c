 ...
 * Copyright (C) Asbjorn Djupdal 2000-2001
 */
...
struct diskParams {
  ...
  int realTrack;    /* hvilken spor r/w-hode star over */
};
...
static boolean motOn = FALSE;       /* om motor er pa */
...
static byte command = NO_COMMAND;   /* kommando som pagar */
    ...
    if (params->active) {
      ...
      if ((sector > params->sectors) || (side > params->sides)) {
        ...
        if (value & 0x10) /* les resten av sektor i spor */
          endOffset = (((params->realTrack+1) << params->sides) + side) *
...
void newDiskData (byte value) {
  ...
  switch (command) {
    case WRITE_SECT:
      if (!params->diskImage) { command = NO_COMMAND; status = 0x80; break; }
