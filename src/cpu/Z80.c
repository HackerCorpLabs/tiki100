...
#include "CodesCB.h"
    default:
      if(R->TrapBadOps)
        printf
        (   
          "[Z80 %" PRIxPTR "] Unrecognized instruction: CB %02X at PC=%04X\n",
          (uintptr_t)(R->User),RdZ80(R->PC.W-1),R->PC.W-2
        );
  }
}
...
#include "CodesXCB.h"
    default:
      if(R->TrapBadOps)
        printf
        (
          "[Z80 %" PRIxPTR "] Unrecognized instruction: DD CB %02X %02X at PC=%04X\n",
          (uintptr_t)(R->User),RdZ80(R->PC.W-2),RdZ80(R->PC.W-1),R->PC.W-4
        );
  }
#undef XX
}
...
#include "CodesXCB.h"
    default:
      if(R->TrapBadOps)
        printf
        (
          "[Z80 %" PRIxPTR "] Unrecognized instruction: FD CB %02X %02X at PC=%04X\n",
          (uintptr_t)R->User,RdZ80(R->PC.W-2),RdZ80(R->PC.W-1),R->PC.W-4
        );
  }
#undef XX
...
#include "CodesED.h"
    ...
    default:
      if(R->TrapBadOps)
        printf
        (
          "[Z80 %" PRIxPTR "] Unrecognized instruction: ED %02X at PC=%04X\n",
          (uintptr_t)R->User,RdZ80(R->PC.W-1),R->PC.W-2
        );
  }
}
...
#include "CodesXX.h"
    ...
    default:
      if(R->TrapBadOps)
        printf
        (
          "[Z80 %" PRIxPTR "] Unrecognized instruction: DD %02X at PC=%04X\n",
          (uintptr_t)R->User,RdZ80(R->PC.W-1),R->PC.W-2
        );
  }
#undef XX
